...
public class ProductsAdapter_team extends RecyclerView.Adapter<ProductsAdapter_team.ProductViewHolder> {
...
String con;
    ...
    private List<Element_jobs> productList;
   // Context mContxt;

    public ProductsAdapter_team(Context mCtx, List<Element_jobs> productList) {
        this.mCtx = mCtx;
    ...
    public ProductViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        View view = inflater.inflate(R.layout.product_list_team, null);
        return new ProductViewHolder(view);
    }

    @Override
    public void onBindViewHolder(final ProductViewHolder holder, final int position) {
     final Element_jobs product = productList.get(position);
...
//            }
        ...
        //loading the image
        Glide.with(mCtx)
                .load(product.job_image())
                .into(holder.imageView);

        holder.textViewTitle.setText(product.job_name());
        holder.textViewShortDesc.setText(product.job_desc());
...
//text=product.getImage();
                ...
                Intent al = new Intent(mCtx, Choice_employ.class);



                int job_id=product.job_id();

                al.putExtra("password",Choice_Project.password);
                al.putExtra("massage_type",Choice_Project.massge_type);
                al.putExtra("project_id",Choice_team.project_id3);
                al.putExtra("job_id",job_id);
